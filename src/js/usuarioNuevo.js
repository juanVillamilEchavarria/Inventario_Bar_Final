...
botonConfirmar.addEventListener("click",envio);
...
function envio(e){
    e.preventDefault();
    if (datos.usuario==="" || datos.contrasena==="" || datos.confirmarContrasena==="" || datos.rol==="") {
        e.preventDefault();
        mensaje("Para avanzar debes llenar todos los campos","ADVERTENCIA");
        return;
    }
    if (datos.contrasena !== datos.confirmarContrasena) {
        e.preventDefault();
        mensaje("Las contraseñas no coinciden","ADVERTENCIA");
        return;
...
confirmarContrasena.addEventListener("change",resultado);
rol.addEventListener("change",resultado);
