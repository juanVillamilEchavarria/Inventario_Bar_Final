...
// botones
const botonCrear = document.querySelector("#btnGuardarCliente");
const nuevo = document.querySelector(".nuevo");
// modales
const modalAgregarProducto = new bootstrap.Modal(document.querySelector('#modalAgregarCliente'));
nuevo.onclick = () => {
    abrirModalAgregarCliente();
}
botonCrear.addEventListener("click", envioCrearCliente);
...
function mensajeCrearCliente (aviso,advertencia="", icono=`<svg  xmlns="http://www.w3.org/2000/svg"  width="74"  height="74"  viewBox="0 0 24 24"  fill="none"  stroke="#b61111"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-alert-circle"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0" /><path d="M12 8v4" /><path d="M12 16h.01" /></svg>`){
   ...
   document.querySelector(".divError")?.remove();
    ...
    // asignar hijos
    const backBody = document.querySelector("#modalAgregarCliente");
...
function envioCrearCliente(e){
    e.preventDefault();

    const {nombreCliente,telefonoCliente,correoCliente} = datosCrearCliente;
    if(nombreCliente==="" || telefonoCliente==="" || correoCliente===""){
        mensajeCrearCliente("Todos los campos son obligatorios","ADVERTENCIA"); 
        return; 
    }
    ...
    .then(data => {
        ...
        if (data.exito) {
            ...
            mensajeCrearCliente(data.mensaje,"ADVERTENCIA");
        }
    })
    .catch(error => {
...
function resultadoCrearCliente(e){
    
    datosCrearCliente[e.target.id] = e.target.value;
    
    console.log(datosCrearCliente); 
}
nombreCliente.addEventListener("change", resultadoCrearCliente);
telefonoCliente.addEventListener("change", resultadoCrearCliente);
correoCliente.addEventListener("change", resultadoCrearCliente);