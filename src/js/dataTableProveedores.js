...
function ajustarScroll() {
    ...
    if (datatableInicio) {
        datatable.destroy();
        datatable = $("#datatable_proveedores").DataTable(datatableParametros);
    }
}
...
const listaProveedores = async function() {
    try {
        const res= await fetch("src/php/funciones/proveedores/tablaProveedores.php", {
            method: "GET",
           credentials: "include"
        })
        const data=await res.json();
            ...
            data.forEach((user) => {
                ...
                contenido += `
                    <tr>
                        ...
                        <td class="text-center tabla-campo">${user.categoria}</td>
                        <td class="text-center tabla-campo">
            ...
            const tableBody_proveedores = document.querySelector("#tableBody_proveedores");
            tableBody_proveedores.innerHTML = contenido;
